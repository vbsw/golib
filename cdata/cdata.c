...
static void cdata_set(cdata_t *const cdata, const char *const id, void *const data) {
	...
	int *const sorts = &cdata[0].props[cdata[0].offs_cap];
	const int last_idx = cdata[0].offs_len - 1;
	const char *const id0 = id ? id : "";
	/* binary search */
	int left = 0, right = last_idx;
	while (left <= right) {
		const int middle = (left + right) / 2;
		const int offs_idx = sorts[middle];
		const char *const id_curr = &ids[offs[offs_idx]];
		...
		if (result < 0) {
			...
			cdata[0].list[offs[offs_idx]] = data;
			break;
		}
	}
	/* add */
	if (left > right) {
		...
		if (cdata[0].offs_len >= cdata[0].offs_cap) {
			const int offs_cap_new = cdata[0].offs_cap * 2;
			int *const props_new = (int*)malloc(sizeof(int) * (size_t)(offs_cap_new*2));
			void **const list_new = (void**)malloc(sizeof(void*) * (size_t)offs_cap_new);
			memcpy(props_new, offs, sizeof(int) * (size_t)cdata[0].offs_len);
			memcpy(list_new, cdata[0].list, sizeof(void*) * (size_t)cdata[0].offs_len);
			if (left > 0)
				memcpy(&props_new[offs_cap_new], sorts, sizeof(int) * (size_t)left);
			if (left < cdata[0].offs_len)
				memcpy(&props_new[offs_cap_new+left+1], &sorts[left], sizeof(int) * (size_t)(cdata[0].offs_len-left));
			free(cdata[0].props);
			free(cdata[0].list);
			cdata[0].props = props_new;
			cdata[0].list = list_new;
			cdata[0].offs_cap = offs_cap_new;
		} else if (left < cdata[0].offs_len) {
			memmove(&sorts[left+1], &sorts[left], sizeof(int) * (size_t)(cdata[0].offs_len-left));
		}
		cdata[0].props[cdata[0].offs_len] = cdata[0].ids_len;
		cdata[0].list[cdata[0].offs_len] = data;
		memcpy(&cdata[0].ids[cdata[0].ids_len], id0, sizeof(char) * (size_t)id0_len);
		cdata[0].ids_len += id0_len;
		cdata[0].props[cdata[0].offs_cap+left] = cdata[0].offs_len;
		cdata[0].offs_len++;
	}
}
...
/*
	if (id) {
		...
		const char *const ids = (const char*)cdata[0].ids;
		int *const offs = cdata[0].props;
		int *const ids_sorts = &cdata[0].props[length];
*/
		/* binary search */
/*
...
void vbsw_cdata_init(const int passes, void **const data, void **const funcs, const int length, long long *const err1, long long *const err2, char **const err_str) {
	if (passes > 0 && length > 0) {
		...
		const int offs_cap = length;
		cdata.props = (int*)malloc(sizeof(int) * (size_t)(offs_cap*2));
		if (cdata.props) {
			const size_t list_size = sizeof(void*) * (size_t)offs_cap;
			cdata.list = (void**)malloc(list_size);
			if (cdata.list) {
				const int ids_cap = length * 60;
				cdata.ids = (char*)malloc(sizeof(char) * (size_t)ids_cap);
				if (cdata.ids) {
					int pass, i;
					cdata.ids_cap = ids_cap;
					cdata.offs_cap = offs_cap;
